...
private val LOW_PRIORITY_IN_OVERLOAD_RESOLUTION = AnnotationModel(
    name = IdentifierEntity("kotlin.internal.LowPriorityInOverloadResolution"),
    params = listOf()
)
...
private class IdlFileConverter(
    ...
    private val hiddenOverloads: Set<IDLOperationDeclaration>,
) {
    ...
    private fun createGetterForDynamic(expression: ExpressionModel, type: TypeModel, key: String): ExpressionModel {
        ...
        return IndexExpressionModel(expression, identifierExpression)
    }
        ...
        return when (this) {
            ...
            is IDLOperationDeclaration -> MethodModel(
                ...
                annotations = buildList {
                   if (this@convertToModel.copy(override = false) in hiddenOverloads)  {
                       add(suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER"))
                       add(LOW_PRIORITY_IN_OVERLOAD_RESOLUTION)
                   }
                },
                open = false,
                body = null
...
fun IDLSourceSetDeclaration.convertToJsModel(hiddenOverloads: Set<IDLOperationDeclaration>): SourceSetModel {
    ...
    return SourceSetModel(
        listOf("<IRRELEVANT>"),
        sources = files.map { IdlFileConverter(it, typeMap, hiddenOverloads).convert() }
    )
}
