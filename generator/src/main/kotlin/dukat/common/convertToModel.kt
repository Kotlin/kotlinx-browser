...
const val GETTER_METHOD_IMPL_PREFIX = "getMethodImplFor"
const val SETTER_METHOD_IMPL_PREFIX = "setMethodImplFor"
...
private val SUPPRESS_UNUSED_PARAMETER_ANNOTATION = suppress("UNUSED_PARAMETER")

private class IdlFileConverter(
    private val fileDeclaration: IDLFileDeclaration,
    private val typeMap: Map<String, NameEntity?>,
    private val dontGenerateTopLevelInternalAccessors: Boolean
) {
    ...
    private fun IDLSetterDeclaration.processAsTopLevel(ownerName: NameEntity): List<TopLevelModel> {
        val privateSetterName = IdentifierEntity(SETTER_METHOD_IMPL_PREFIX + (ownerName as IdentifierEntity).value)
        ...
        return listOfNotNull(privateSetterImpl.takeIf { !dontGenerateTopLevelInternalAccessors }, publicSetter)
    }
    ...
    private fun IDLGetterDeclaration.processAsTopLevel(ownerName: NameEntity): List<TopLevelModel> {
        ...
        IdentifierEntity(
            (valueType.toNullableIfNotPrimitive().convertToModel() as? TypeValueModel)?.value?.staticConverter()
                ?: "getAny"
        )

        val privateGetterName = IdentifierEntity(GETTER_METHOD_IMPL_PREFIX + (ownerName as IdentifierEntity).value)
        ...
        return listOfNotNull(privateGetterImpl.takeIf { !dontGenerateTopLevelInternalAccessors }, publicGetter)
    }
    ...
    fun convert(): SourceFileModel {
        ...
        val module = ModuleModel(
            ...
            imports = mutableListOf(
                ...
                ImportModel("org.w3c.undefined".toNameEntity()),
            ),
            comment = null
        )
...
fun IDLSourceSetDeclaration.convertToWasmModel(dontCreateTopLevelInternalAccessors: Boolean = false): SourceSetModel {
    ...
    return SourceSetModel(
        listOf("<IRRELEVANT>"),
        sources = files.map { IdlFileConverter(it, typeMap, dontCreateTopLevelInternalAccessors).convert() }
    )
}
