...
class UnionParametersOverloadsLowering(
    private val addDynamicOverload: Boolean,
    private val hiddenOverloads: MutableSet<IDLOperationDeclaration>
) : IDLLowering {
    ...
    private fun generateOverloads(op: IDLOperationDeclaration): List<IDLOperationDeclaration> {
        ...
        val result = mutableListOf<IDLOperationDeclaration>()


        if (addDynamicOverload) {
            val optionalParameters = unionTypedArgument.run {
                copy(type = IDLSingleTypeDeclaration("dynamic", null, true))
            }
            result += op.copy(arguments = op.arguments.subList(0, unionTypedArgumentIndex) + optionalParameters + op.arguments.subList(unionTypedArgumentIndex + 1, op.arguments.size))
                .also { hiddenOverloads += it }
        } else {
            unionMembersFlattened.mapNotNullTo(result) { unionMember ->
        ...
        if (unionTypedArgument.optional || unionTypedArgument.defaultValue != null) {
            result += op.copy(arguments = op.arguments.subList(0, unionTypedArgumentIndex))
        }
...
fun IDLSourceSetDeclaration.addOverloadsForUnions(
    addDynamicOverload: Boolean = false,
    hiddenOverloads: MutableSet<IDLOperationDeclaration> = mutableSetOf()
): IDLSourceSetDeclaration {
    return UnionParametersOverloadsLowering(addDynamicOverload, hiddenOverloads).lowerSourceSetDeclaration(this)
}